var tokenAction = tokenAction || (function() {
    ...
    var version = '0.3.1',
        ...
        createRepeatingMenu = function(name, pattern, id, usename, desc) {
            var macName = desc;
			var macHeader = "/w @{character_name} &{template:atk} {{desc=**" + desc + "**";
			var macBody = "";
            ...
            _.each(repeatingAttrs, function(attr) {
                ...
                var repeatingName = attr.get('current');
                var repeatingAction = getRepeatingAction(id, pattern.replace(/%%RID%%/g, repeatingId), usename);
				var macAction = "~" + (pattern.replace(/%%RID%%/g,repeatingId));
				...
				macBody += "\n[" + repeatingName + "](" + macAction +")";

            });
			
			if (macBody != ""){
				macBody = macHeader + macBody + "}}"
				createObj("ability", {name: macName, action: macBody, characterid: id, istokenaction: true});
			}
		...
		deleteAddedMenu = function(id, char, macro) {
        ...
        createSpell = function(id) {
			...
			var spellSlots = "";
			if (e != "Cantrips"){
				var lvl = e.substring(0,1);
				spellSlots =  "(@{selected|lvl" + lvl + "_slots_expended} of @{selected|lvl" + lvl + "_slots_total})";
			}
				spellText += "**" + e + ":** " + spellSlots + "\n" + sb[e].join(' | ') + "\n\n";
                //spellText += "**" + e + ":**" + "\n" + sb[e].join(' | ') + "\n\n";
            });
			
			var actionHeader = "/w @{character_name} &{template:atk} {{desc=**SPELLS** - Save DC ([[8+@{selected|intelligence_mod}+@{selected|pb}]])\n\n";
			var spellAction = actionHeader + spellText + "}}";
            ...
            if (checkAbility[0]) {
                ...
                createObj("ability", {
                    name: 'Spells',
                    action: spellAction, //"/w @{character_name} &{template:atk} {{desc=" + spellText + "}}",
                    characterid: id,
                    istokenaction: true
                });
            }
        ...
        handleInput = function(msg) {
            ...
            if (msg.content.search(/^(!ta|!sortta)\b/) !== -1) {
                ...
                if (args.includes('pc')) {
					if (args.includes('breakout')) {
						args = [baseCommand, 'attacks', 'spells', 'checks', 'saves', 'reactions', 'init', 'traits', 'breakout'];
					} else {
						args = [baseCommand, 'attacks', 'spells', 'checks', 'saves', 'reactions', 'init', 'traits'];
					}
				}
                ...
                if (args.length === 2 && args.includes('breakout')) {
					args = [baseCommand, 'attacks', 'spells', 'checks', 'saves', 'reactions', 'init', 'traits', 'breakout'];
                }
                    ...
                    if (parseInt(isNpc(a.id)) === 1) {
                        ...
                        if (args.includes("attacks")) {
							...
							if (args.includes("breakout")) {
								...
								var type = "Attacks";
								deleteAddedMenu(a.id, char, type);							
								createRepeatingMenu(/repeating_npcaction_[^_]+_name\b/, 'repeating_npcaction_%%RID%%_npc_action', a.id, usename, type);
							}
                        }
                        if (args.includes("attacks")) {
                            //Legendary Actions
							if (args.includes("breakout")) {
								...
								var type = "Legendary";
								deleteAddedMenu(a.id, char, type);							
								createRepeatingMenu(/repeating_npcaction-l_[^_]+_name\b/, 'repeating_npcaction-l_%%RID%%_npc_action', a.id, usename, type);
							}
                        ...
                        if (args.includes("traits")) {
							...
							if (args.includes("breakout")) {
								...
								var type = "Traits";
								deleteAddedMenu(a.id, char, type);							
								createRepeatingMenu(/repeating_npctrait_[^_]+_name\b/, 'repeating_npctrait_%%RID%%_-npc_roll_output', a.id, usename, type);
							}
                        }
                        if (args.includes("reactions")) {
							...
							if (args.includes("breakout")) {
								...
								var type = "Reactions";
								deleteAddedMenu(a.id, char, type);							
								createRepeatingMenu(/repeating_npcreaction_[^_]+_name\b/, 'repeating_npcreaction_%%RID%%_-npc_roll_output', a.id, usename, type);
							}
                        }
                        ...
                        if (args.includes("attacks")) {
							...
							if (args.includes("breakout")) {
								...
								var type = "Attacks";
								deleteAddedMenu(a.id, char, type);							
								createRepeatingMenu(/repeating_attack_[^_]+_atkname\b/, 'repeating_attack_%%RID%%_attack', a.id, usename, type);
							}
                        }
                        if (args.includes("traits")) {
                            //traits
							if (args.includes("breakout")) {							
								...
								var type = "Traits";
								deleteAddedMenu(a.id, char, type);							
								createRepeatingMenu(/repeating_traits_[^_]+_name\b/, 'repeating_traits_%%RID%%_output', a.id, usename, type);
							}
